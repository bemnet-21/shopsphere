      ...
      <Link href={`/product/${id}`}>
        <div className='relative w-36 h-36 bg-darkBlue rounded-xl transform transition-all duration-150 hover:scale-105 active:scale-100 active:brightness-90'>
          ...
          <div className='flex flex-col w-1/2 gap-y-2'>
              <div>Name: {title}</div>
              <div>Price: ${price}</div>
              <button className='bg-mainOrange rounded-3xl cursor-pointer hover:bg-amber-700 active:bg-amber-800'>Buy Now</button>
              <button className='border border-[#FF0000] text-[#FF0000] rounded-3xl hover:bg-[#ff0000] hover:text-[#fff] active:bg-[#DC2626] active:border-[#DC2626] active:text-white' onClick={handleRemove}>Remove</button>
          </div>
    </div>
  )
}
